...
namespace RiSC.MainPageMVVM
{
    internal class MainPageViewModel : INotifyPropertyChanged
    {
        ...
        public async void ModelDescription()
        {
            ...
            var Reciviedmsg = (await Client.GetAsync(Client.BaseAddress)).Content.ReadAsStream();
            do
            {
        ...
        public async void GetSampler()
        {
            ...
            int NumberOfBytes = 0;
            byte[] bytes = new byte[1024];
            var Reciviedmsg = (await Client1.GetAsync(Client1.BaseAddress)).Content.ReadAsStream();
            do
            {
                NumberOfBytes = Reciviedmsg.Read(bytes, 0, bytes.Length);
                builder.Append(Encoding.UTF8.GetString(bytes, 0, NumberOfBytes));
        ...
        public async void Generateimg()
        {
            ...
            var json = (await client.PostAsync(client.BaseAddress, data)).Content.ReadAsStream();
            byte[] bytes = new byte[1024];
            StringBuilder builder = new StringBuilder();
            int numofbytes = 0;
            do
